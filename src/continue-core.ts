/** Shared core: platform-independent pure logic and types.
 *
 * Ported from `dsh-client-auto-continue` (MIT, Copyright (c) 2025 HsiangNianian)
 * `src/shared/core.ts` into `dsh-failover-continue`, with adaptations:
 * locales RU/EN (was ZH/EN), live-tuned defaults (scan window, backoff cap,
 * consecutive budget), and a `continuable` seam so the failover circuit can
 * mark route-switched failures as worth continuing (AUTH/BILLING included).
 *
 * Shared by the host engine and the browser half: config resolution, error
 * classification, template filling, adaptive backoff, idempotency-guard
 * helpers, loop-guard state machine, echo recognition.
 */
import type { SessionEvent } from '@deepseek-ai/dsh-session/types';

/** Supported UI/config locales. Any unknown browser locale falls back to Russian. */
export type AutoContinueLocale = 'ru' | 'en';
...
export function resolveConfig(section: AutoContinueSettings | undefined): AutoContinueConfig {
  ...
  const locale: AutoContinueLocale = value.locale === 'en' ? 'en' : 'ru';
...
/** One day of auto-continue stats (host single-instance memory). */
