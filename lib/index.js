...
import { CircuitBreaker, modelKey, stepUpIndex } from "./circuit.js";
...
export function apply(ctx, entry = {}) {
    ...
    const trailScanned = new WeakSet();
     ...
     */
    const reconstructTrailFromLog = (agent) => {
        let events;
        try {
        ...
        for (const event of tail) {
            ...
            if (event.seq <= (lastOurs?.seq ?? -1))
                continue;
            // Anything routing-related after our notice voids the trail.
            if (text.startsWith('[model changed:'))
                return;
            if (data?.source?.kind === 'user' && data?.source?.rpcId !== undefined)
                return;
        }
        if (lastOurs === undefined)
            return;
        let lastHeader;
        for (const event of tail) {
            if (event.type !== 'request/header' || event.seq < lastOurs.seq)
                continue;
        ...
        if (lastHeader === undefined)
            return;
        if (lastHeader.provider !== lastOurs.provider || lastHeader.model !== lastOurs.model)
            return;
    ...
    ctx.on('agent/request', async (payload, next) => {
        ...
        const pos = list.findIndex((route) => route.provider === primary.provider && route.model === primary.model);
            ...
            if (onTrail) {
                ...
                if (up >= 0) {
                    ...
                    if (target !== undefined) {
                        ...
                        console.warn('[dsh-failover-continue] %s: %s/%s stepping up to %s/%s (priority)', payload.agent.id, primary.provider, primary.model, target.provider, target.model);
