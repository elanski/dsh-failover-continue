...
export function apply(ctx, entry = {}) {
    ...
    ctx.on('agent/request', async (payload, next) => {
        ...
        try {
            ctx.logger.warn('[dsh-failover-continue] route t%d/s%d base=%s/%s home=%s/%s open=%s diverted=%s', payload.turn, payload.step, primary.provider, primary.model, home.provider, home.model, breaker.isOpen(home.provider, home.model) ? 'Y' : 'n', reverted.has(payload.agent) ? 'Y' : 'n');
        }
