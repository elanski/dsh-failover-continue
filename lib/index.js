...
export function apply(ctx, entry = {}) {
    ...
    ctx.on('agent/request', async (payload, next) => {
        ...
        if (diverted.has(payload.agent) && homeKey !== primaryKey && !breaker.isOpen(home.provider, home.model)) {
            ...
            ctx.logger.warn('[dsh-failover-continue] %s: %s/%s recovered, reverting to %s/%s', payload.agent.id, primary.provider, primary.model, home.provider, home.model);
