...
import { RECOVERY_WINDOW_MS, effectiveCooldown, emptyDayStats, fillTemplate, forgetPendingEcho, freshState, isNonHumanReason, isOurEcho, isTransientFailure, shouldNudgeIdle, sleep, todayKey, trackPendingEcho, } from "./continue-core.js";
const NOTICE_COPY = {
    ru: {
        notContinuedTitle: 'failover-continue: не продолжено',
        permanentErrorBody: (sessionId, summary) => `${sessionId}: постоянная ошибка ${summary}, нужно вмешательство человека`,
        ...
        continuedBody: (sessionId, text, count) => `${sessionId}: отправлено «${text}» (подряд №${count})`,
        stoppedTitle: 'failover-continue: автопродолжение остановлено',
        stoppedBody: (sessionId, count) => `${sessionId}: ${count} провалов подряд, нужно вмешательство человека`,
    },
    en: {
 ...
 */
function openTurnStartSeq(events) {
    let lastStart = -1;
    let lastEnd = -1;
 ...
 */
function looksInterrupted(events) {
...
export class AutoContinueRunner {
    ...
    async maybeNudgeIdle(candidate, lastEnd, now, config) {
        ...
        this.idleNudged.set(candidate.sessionId, entry !== undefined && entry.date === today
     ...
     */
    constructor(ctx, getConfig, options) {
        this.ctx = ctx;
        ...
        this.disposeSessionEvents = ctx.on('session/event', (session, event) => {
            try {
                ...
                console.error(`[auto-continue] 会话事件处理异常 ${session.id}: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    ...
    handleNoticeAction(sessionId, action) {
        if (action === 'unpause') {
            if (sessionId !== undefined)
                this.pauseUntil.delete(sessionId);
    ...
    onHostEvent(session, event) {
        ...
        if (event.type === 'tool/call') {
            ...
            if (repeat !== undefined)
                this.checkLoop(sessionId, state, repeat);
        }
    ...
    isNearDuplicateSegment(left, right) {
        ...
        if (shorter === 0 || shorter / longer < 0.85)
            return false;
        if (longer > STREAM_NEAR_DUPLICATE_MAX_CHARS)
            return false;
        const maxDistance = Math.max(6, Math.floor(longer * 0.08));
        if (Math.abs(leftLen - rightLen) > maxDistance)
            return false;
    ...
    withinEditDistance(left, right, maxDistance) {
        ...
        if (leftLen === 0 || rightLen === 0)
            return Math.max(leftLen, rightLen) <= maxDistance;
        ...
        for (let row = 1; row <= leftLen; row += 1) {
            ...
            for (let col = firstCol; col <= lastCol; col += 1) {
                ...
                const substitution = (previous[col - 1] ?? unreachable) +
    ...
    onAssistantChunk(sessionId, state, event) {
        if (!this.getConfig().loopGuard || !state.running || state.loopFired)
            return;
        ...
        for (const piece of pieces)
            this.noteStreamSegment(sessionId, state, piece);
    ...
    onAssistantMessage(sessionId, state, event) {
        ...
        state.streamTail = '';
        if (streamTail !== '')
            this.noteStreamSegment(sessionId, state, streamTail);
        state.streamLastSegment = '';
        state.streamRepeatRun = 0;
    ...
    onSessionEvent(sessionId, event) {
        ...
        switch (event.type) {
            ...
            case 'user/message':
                if (isOurEcho(state, event))
                    break; // 我们自己的回显(跨标签页识别)
     ...
     */
    isWorthContinuing(failure) {
        const config = this.getConfig();
        if (!config.classify)
            return true;
        if (isTransientFailure(failure, config.retryableErrorPatterns))
            return true;
        try {
    ...
    onTurnFailure(sessionId, reason, failure) {
        ...
        if (!this.isWorthContinuing(failure)) {
            ...
            if (config.notify) {
                this.notify(sessionId, copy.notContinuedTitle, copy.permanentErrorBody(sessionId, summary), this.notifyOptions(sessionId, config.locale));
            }
            return;
        }
    ...
    bumpStat(delta) {
        ...
        if (this.dayStats.date !== today)
            this.dayStats = emptyDayStats();
        if (delta.sent !== undefined)
            this.dayStats.sent += delta.sent;
        if (delta.skipped !== undefined)
            this.dayStats.skipped += delta.skipped;
        if (delta.recovered !== undefined)
            this.dayStats.recovered += delta.recovered;
        if (delta.failed !== undefined)
            this.dayStats.failed += delta.failed;
        if (delta.gaveUp !== undefined)
            this.dayStats.gaveUp += delta.gaveUp;
        if (delta.looped !== undefined)
            this.dayStats.looped += delta.looped;
        if (delta.nudged !== undefined)
            this.dayStats.nudged += delta.nudged;
    ...
    schedule(sessionId, reason, textOverride) {
        const state = this.state(sessionId);
        const config = this.getConfig();
    ...
    async fire(sessionId, reason, force = false, textOverride) {
        if (this.disposed)
            return;
        const state = this.state(sessionId);
        const config = this.getConfig();
    ...
    async scanLoop(attempts, delayMs) {
        for (let attempt = 0; attempt < attempts && !this.disposed; attempt += 1) {
            try {
                ...
                if (attempt % 10 === 0) {
                    this.log(`扫描失败(${attempt + 1}/${attempts === Infinity ? '∞' : attempts}): ${error instanceof Error ? error.message : String(error)}`);
                }
            }
    ...
    async scanInterrupted() {
        ...
        if (controller !== undefined && typeof controller.list === 'function') {
            ...
            for (const item of items) {
                if (inspectedCount >= config.scanLimit)
                    break;
                ...
                if (liveIds.has(sessionId))
                    continue;
                if (item.parentSessionId !== undefined && item.origin === 'subagent' && item.running === true)
                    continue;
                const updatedAt = typeof item.updatedAt === 'number' ? item.updatedAt : 0;
                if (updatedAt > 0 && updatedAt < now - config.freshMs)
                    continue;
                // 便宜的状态检查放在 inspect 之前: inspect 会整读(并解压)日志, 很贵。
                const cheap = this.state(sessionId);
                if (cheap.pendingTimer !== undefined)
                    continue;
                if (cheap.consecutive >= config.maxConsecutive)
                    continue;
                if (now - cheap.lastAttemptAt < this.cooldownFor(cheap))
                    continue;
                if (now < (this.pauseUntil.get(sessionId) ?? 0))
                    continue;
                if (now - (this.lastInspectAt.get(sessionId) ?? 0) < COLD_INSPECT_TTL_MS)
                    continue;
                // 判定缓存: inspect() 整读并解压日志(可达数 MB)。若上一次读到的 updatedAt
                // 与现在相同, 说明日志没变, 结论必然一样 —— 直接复用, 不要重复读盘。
                const cached = this.coldVerdict.get(sessionId);
                if (cached !== undefined && cached.updatedAt === updatedAt && !cached.resumable)
                    continue;
                this.lastInspectAt.set(sessionId, now);
                inspectedCount += 1;
                let events;
                try {
                    const inspect = controller.inspect;
                    const inspected = await inspect?.(sessionId);
                ...
                for (let i = events.length - 1; i >= 0; i -= 1) {
                    ...
                    if (event !== undefined && event.type === 'turn/end') {
                        lastKind = readReasonKind(event.data.reason);
                        break;
                    }
                }
                const worthChecking = looksInterrupted(events) || lastKind === 'completed';
                this.coldVerdict.set(sessionId, { updatedAt, resumable: worthChecking });
                if (!worthChecking)
                    continue;
                candidates.push({
                    sessionId: sessionId,
        ...
        for (const candidate of candidates.slice(0, config.scanLimit)) {
            ...
            if (candidate.running)
                continue; // 回合正在跑: 不是中断, 不干预
            const state = this.state(candidate.sessionId);
            if (state.pendingTimer !== undefined)
                continue;
            if (state.consecutive >= config.maxConsecutive)
                continue;
            if (now - state.lastAttemptAt < this.cooldownFor(state))
                continue;
            if (now < (this.pauseUntil.get(candidate.sessionId) ?? 0))
                continue; // 会话暂停中
            ...
            if (openTurn) {
                ...
                if (reasonKind === 'completed') {
                    ...
                    if (await this.maybeNudgeIdle(candidate, lastEnd, now, config))
                        scheduled += 1;
                    continue;
                }
                if (reasonKind === undefined || !isNonHumanReason(reasonKind)) {
                    if (candidate.cold)
                        this.coldVerdict.set(candidate.sessionId, { updatedAt: candidate.coldUpdatedAt, resumable: false });
                    continue;
                }
            }
            ...
            const inScope = (event) => openTurn ? event.seq >= boundarySeq : event.seq > boundarySeq;
            let superseded = false;
            for (const event of events) {
                if (!inScope(event))
                    continue;
                // 孤儿回合被后来补上的 turn/end 收尾 = 已经处理过了
                if (openTurn && event.type === 'turn/end')
                    superseded = true;
                // 已收尾回合之后又开了新回合 = 已经处理过了
                if (!openTurn && event.type === 'turn/start')
                    superseded = true;
            ...
            this.log(`扫描发现中断 ${candidate.sessionId}(turn/end:${reasonKind}${candidate.cold ? ', 冷会话' : ''}), 交给恢复策略处理`);
            if (reasonKind === 'error' && lastEnd !== undefined) {
                const failure = parseFailureFacts(lastEnd.data.reason.error);
                if (failure === undefined) {
    ...
    sessionController() {
        try {
            return this.ctx.get('sessionController', false);
        }
        catch {
            return undefined;
        }
    ...
    async ensureLiveAgent(sessionId) {
        ...
        try {
            ...
            if (found !== undefined && found !== null && found.agent !== undefined)
                return found.agent;
        }
        catch (error) {
            this.log(`唤醒会话失败 ${sessionId}: ${error instanceof Error ? error.message : String(error)}`);
        }
